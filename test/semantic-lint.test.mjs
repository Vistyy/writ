...
import { MODEL, QUESTIONS, classify, discoverSkills, main, readSkillMetadata, runSemanticLint } from "../src/semantic-lint.mjs";
...
test("CLI boundary does not construct a TypeSafe client when routing has no work", async (t) => {
  const root = await fixture({ "elsewhere/SKILL.md": "ignored\n" });
  t.after(() => rm(root, { recursive: true, force: true }));
  let factories = 0;
  const out = output();
  assert.equal(await main({ root, createClient: async () => { factories++; throw new Error("no credentials"); }, stdout: out.write, stderr: out.write }), 0);
  assert.equal(factories, 0);
  assert.equal(out.lines.at(-1), "RECEIPT model=none input_tokens=0 output_tokens=0 evaluated=0 skipped=0");
});
